...
class reserve:
    def __init__(
        ...
        # 设置超时时间，避免长时间等待
        self.requests.timeout = (3, 5)  # (连接超时, 读取超时)
    ...
    def login(self, username, password):
        ...
        try:
            ...
            if obj["status"]:
                ...
                logging.info(f"User {username} login failed. Please check password and username!")
    ...
    def resolve_captcha(self):
        ...
        try:
            ...
            logging.error(f"Error in resolve_captcha: {e}")
            return ""
    ...
    def submit_single_seat(self, times, roomid, seat, action):
        ...
        logging.info(f"Get token for seat {seat}: {token}")
        ...
        if len(seatid) == 1:
            ...
            while attempt < self.max_attempt:
                try:
                    ...
                    logging.error(f"Error submitting seat {seat}: {e}")
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(seatid), 4)) as executor:
                for attempt in range(self.max_attempt):
                    futures = []
                    for seat in seatid:
                        future = executor.submit(self.submit_single_seat, times, roomid, seat, action)
                    ...
                    for future in concurrent.futures.as_completed(futures, timeout=10):
                        try:
                            ...
                            logging.error(f"Error in concurrent submit: {e}")
